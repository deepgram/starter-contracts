...
test.describe('Flux Text-to-Speech UI Workflow', () => {
  test('should connect, send text, and disconnect from Flux TTS', async ({ page }) => {
    // 1. Navigate to app
    try {
    ...
    // 4. Click Send button to trigger TTS
    try {
      await page.getByRole('button', { name: ' Send' }).click();
    } catch (error) {
      throw new Error(
        `❌ Could not click Send button\n` +
        `💡 Likely cause: Button not found or disabled\n` +
        `🔧 How to fix:\n` +
        `   1. Check that Send button has proper role and name\n` +
        `   2. Verify button is enabled after connection\n` +
        `Original error: ${error.message}`
      );
    }

    // 5. Verify transcript shows the expected text
    try {
      await expect(page.locator('#transcript-container')).toContainText(
        'Hello! This is a test of Flux streaming text-to-speech. The audio should play smoothly as it\'s generated.',
        { timeout: 15000 }
      );
    } catch (error) {
      throw new Error(
        `❌ Transcript did not show expected text\n` +
        `💡 Likely cause: TTS stream not received or transcript not updating\n` +
        `🔧 How to fix:\n` +
        `   1. Check that backend is forwarding TTS responses\n` +
        `   2. Verify Deepgram TTS API is responding\n` +
        `   3. Check browser console for streaming errors\n` +
        `   4. Ensure transcript container updates on data received\n` +
        `Original error: ${error.message}`
      );
    }
    ...
    // 7. Verify connection status shows "Disconnected"
    try {
  ...
  test('should send text and display in transcript', async ({ page }) => {
    // Navigate to app
    await page.goto(BASE_URL);
    ...
    // Verify transcript shows the sent text
    try {
      await expect(page.locator('#transcript-container')).toContainText(
        'Hello! This is a test of Flux streaming text-to-speech. The audio should play smoothly as it\'s generated.',
        { timeout: 15000 }
  ...
  test('should have required UI elements', async ({ page }) => {
    ...
    // Note: Send button and transcript container may be hidden until connected
  });
});
