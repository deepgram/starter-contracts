...
# Run Text-to-Speech test harness
# This script is designed to be run from a starter app directory (e.g., node-text-to-speech)
...
#
# Usage: ./contracts/tests/run-text-to-speech-app.sh
...
echo "✅ App is running"
echo ""
echo "Running Text-to-Speech test harness"
echo "====================================================="
...
if ./contracts/tests/text-to-speech/structure/validate-contracts.sh "$REPO_ROOT"; then
...
if ./contracts/tests/text-to-speech/structure/validate-frontend.sh "$REPO_ROOT"; then
...
if npm run test:text-to-speech -- tests/text-to-speech/api/; then
...
if NPM_CONFIG_LOGLEVEL=error npx playwright test tests/text-to-speech/ui/workflow.spec.js; then
