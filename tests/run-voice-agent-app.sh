...
# Run Voice Agent test harness
# This script is designed to be run from a starter app directory (e.g., node-voice-agent)
...
#
# Usage: ./contracts/tests/run-voice-agent-app.sh
...
echo "✅ App is running"
echo ""
echo "Running Voice Agent test harness"
echo "====================================================="
...
if ./contracts/tests/voice-agent/structure/validate-contracts.sh "$REPO_ROOT"; then
...
if ./contracts/tests/voice-agent/structure/validate-frontend.sh "$REPO_ROOT"; then
...
if npm run test:voice-agent -- tests/voice-agent/api/; then
...
if npx playwright test tests/voice-agent/ui/workflow.spec.js; then
