...
test.describe('Live Text-to-Speech UI Workflow', () => {
  test('should connect, send text, and disconnect from live TTS', async ({ page }) => {
    // 1. Navigate to app
    try {
    ...
    try {
      await expect(page.locator('#transcript-container')).toContainText(
        'Hello! This is a test of streaming text-to-speech. The audio should play smoothly as it\'s generated.',
        { timeout: 15000 }
      );
    } catch (error) {
      throw new Error(
